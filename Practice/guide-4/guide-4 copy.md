...
fn main () {
      ...
      x = 2; // No se puede reasignar una variable inmutable
      println (“El valor de x es {}”, x);
}
```
```rust
...
fn main () {
      let x = 1;
      println (“El valor de x es {}”, x);
      let x = 2; // Es shadowing se tapa la declaracion anterior con una nueva
      println (“El valor de x es {}”, x);
}
```
...
// Programa 4
fn main () {
      let num = Some(7); // Defino una variable num de tipo Option<i32>, con valor Some(7)
      if let Some(x) = num { // Si num es Some, extraigo el valor y lo asigno a x
             println! (“Match: {:?}”, x);
